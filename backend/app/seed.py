...
def seed():
 ...
 try:
  ...
  if not u:u=User(name='Amina',email='student@demo.com',password_hash=PasswordHash.recommended().hash('Demo123!'),country='Kazakhstan');db.add(u);db.flush()
  p=db.query(StudentProfile).filter_by(user_id=u.id).first()
  if not p:p=StudentProfile(user_id=u.id,birth_year=date.today().year-17,country='Kazakhstan',city='Almaty',education_level='HIGH_SCHOOL',grade_year='11',gpa=3.6,gpa_scale=4,english_level='B2',budget_level='LOW',preferred_countries=json.dumps(['Germany','Netherlands','International']),preferred_fields=json.dumps(['Chemical Engineering']),skills=json.dumps(['Chemistry','Mathematics']),interests=json.dumps(['Sustainability']),achievements='Regional chemistry finalist',volunteering='Community tutoring',extracurriculars='Science club');db.add(p);db.flush()
  g=db.query(Goal).filter_by(user_id=u.id,status='ACTIVE').first()
  if not g:g=Goal(user_id=u.id,title='Chemical Engineering Abroad',description='Study Chemical Engineering in Europe in English with substantial financial support.',target_field='Chemical Engineering',target_countries=json.dumps(['Germany','Netherlands']),funding_requirement='HIGH',education_level='BACHELOR',language='English',target_date=date.today()+timedelta(days=365));db.add(g);db.flush()
  ...
  sync_gaps(db,u.id,g,p);readiness(db,u.id,g,p);generate_roadmap(db,u,g);db.commit();print(f'Seed complete: {db.query(Opportunity).count()} opportunities, demo user {u.email}')
 finally:db.close()
