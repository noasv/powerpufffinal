...
def opportunity_view(profile,goal,opp,gaps,ready):
    ...
    return {'id':opp.id,'title':opp.title,'provider':opp.provider,'opportunity_type':opp.opportunity_type,'description':opp.description,'fields':fields,'country':opp.country,'delivery_mode':opp.delivery_mode,'funding_type':opp.funding_type,'funding_amount_text':opp.funding_amount_text,'deadline':opp.deadline,'official_url':opp.official_url,'source_url':opp.source_url,'source_domain':opp.source_domain,'source_label':opp.source_label,'source_type':opp.source_type,'verification_status':opp.verification_status,'discovered_at':opp.discovered_at,'verified_at':opp.verified_at,'canonical_source_url':getattr(opp,'canonical_source_url',None) or opp.official_url,'discovery_source_url':getattr(opp,'discovery_source_url',None),'source_quality':getattr(opp,'source_quality',None),'is_first_party':getattr(opp,'is_first_party',False),'verification_notes':getattr(opp,'verification_notes',None),'last_checked_at':getattr(opp,'last_checked_at',None),'requirements_text':opp.requirements_text,'match_score':score,'field_relevance':relevance,'readiness_score':ready['overall'],'eligibility_status':status,'eligibility_reasons':reasons,'gap_impact':strength,'gap_impact_score':impact_score,'impacts':impacts,'explanation':explanation}
