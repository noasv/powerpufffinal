...
export type Opportunity={id:number;title:string;provider:string;opportunity_type:string;description:string;fields:string[];country:string;delivery_mode:string;funding_type:string;funding_amount_text?:string|null;deadline:string|null;match_score:number;readiness_score:number;gap_impact:string;eligibility_status:string;explanation:string;impacts:{title:string;strength:string;reason:string}[];official_url:string;source_url:string;source_domain:string|null;source_label:string;source_type:string;verification_status:'VERIFIED'|'SOURCE_FOUND'|'UNVERIFIED'|'DEMO';discovered_at?:string|null;verified_at?:string|null;canonical_source_url?:string|null;discovery_source_url?:string|null;source_quality?:string|null;is_first_party?:boolean;verification_notes?:string|null;last_checked_at?:string|null;requirements_text:string;eligibility_reasons:string[]}
